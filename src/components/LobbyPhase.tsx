      ...
      <header className="h-14 bg-[#16161a] border-b border-[#2a2a32] flex items-center justify-between px-6 shrink-0">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 px-3 py-1 bg-black/40 rounded-lg border border-[#2a2a32]">
            <span className="text-[10px] text-gray-600 font-black uppercase">Room ID</span>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-72 border-r border-[#2a2a32] bg-[#111114] flex flex-col overflow-y-auto">
          ...
          <div className="p-5 space-y-6">
            <section>
              <label className="block text-[10px] font-black text-gray-600 uppercase tracking-widest mb-2">最大参加人数</label>
              <select 
            ...
            <section>
              <label className="block text-[10px] font-black text-gray-600 uppercase tracking-widest mb-2">NPCの参戦</label>
              <div className="flex items-center justify-between p-3 bg-[#1c1c21] border border-[#2a2a32] rounded-xl">
                <span className="text-xs font-bold">{roomSettings.npcEnabled ? 'ON (自動補充)' : 'OFF (対人のみ)'}</span>
                <button 
            ...
            <section>
              <label className="block text-[10px] font-black text-gray-600 uppercase tracking-widest mb-2">所持金ルール</label>
              <div className="grid grid-cols-1 gap-2">
                {[
                  ...
                  >
                    ...
                    <div className="text-[9px] opacity-60 font-bold mt-0.5">{r.desc}</div>
                  </button>
                ))}
              </div>
            </section>
            <section>
              <label className="block text-[10px] font-black text-gray-600 uppercase tracking-widest mb-2">ホスト自動移譲</label>
              <div className="flex items-center justify-between p-3 bg-[#1c1c21] border border-[#2a2a32] rounded-xl">
                <span className="text-[10px] text-gray-400 font-bold leading-tight mr-4">ホスト切断時に次のプレイヤーが引き継ぐ</span>
                <button 
          ...
          <div className="flex-1 overflow-y-auto p-8">
            <div className="max-w-xl mx-auto">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xs font-black text-gray-400 uppercase tracking-[0.2em]">Participants</h3>
                <span className="text-[10px] font-mono text-gray-600 tabular">{participants.length} / {roomSettings.participantLimit} Players</span>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          <span className="text-[10px] text-gray-600 font-bold uppercase tracking-widest">Connected</span>
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-[10px] font-black text-gray-600 uppercase tracking-widest mb-0.5">Balance</div>
                ...
                {participants.length < roomSettings.participantLimit && Array.from({ length: Math.min(3, roomSettings.participantLimit - participants.length) }).map((_, i) => (
                  <div key={`empty-${i}`} className="border-2 border-dashed border-[#2a2a32] rounded-2xl px-6 py-4 flex items-center justify-center opacity-30">
                    <span className="text-[10px] font-black text-gray-600 uppercase tracking-[0.3em]">Empty Slot</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="h-24 border-t border-[#2a2a32] bg-[#16161a] flex items-center justify-center px-8">
            {isHost ? (
              ...
              <div className="text-center animate-pulse">
                <span className="text-sm font-black text-gray-600 uppercase tracking-[0.4em]">Waiting for Host to start...</span>
              </div>
            )}
          </div>
        </div>
