                ...
                {[
                  {
                    ...
                    details: [
                      'レース描画を刷新し、馬／丸の表示切替、複数カメラ、注目馬の追跡に対応。',
                      'ゲートインから発走までの配置と切替を安定化し、レース映像へのつながりを改善。',
                      '際どい決着では全馬入線後に写真判定を実施。実際の決勝線通過時の位置関係をそのまま再現します。',
                      '結果画面に、レース全編を連続再生できるハイライトリプレイと重要場面への移動機能を追加。',
                      '実況を実際の進行率・順位変動・馬群の形に連動させ、団子、縦長、先頭への圧力、後方一気などの展開実況を追加。',
                      ...
                      '中央競馬場のテスト運用を開始。5分ごとの定時開催、800頭規模の中央馬プール、中央専用残高、締切までの馬券変更、未観戦時の自動精算に対応。',
